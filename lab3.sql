...
------------------------------------1-------------------------------------------
---a
select distinct title,dept_name,course_id,credits
from course
where credits > 3;
---b
...
select distinct title
from course,teaches
where course.course_id=teaches.course_id and semester='Fall';
---e
select name
from student
...
from student,id_arr
where student.id = id_arr.id and dept_name='Comp. Sci.' and number>3;
---e
select name
from instructor
...
select distinct instructor.id,name,in_2018.year
from instructor,in_2018
where instructor.id = in_2018.id and instructor.id not in (
    select distinct id
    from teaches
    where year=2017
    );
--------------------------------------------3--------------------------------------------
---a
with comp_students as(
    select id,name
    from student
    where dept_name='Comp. Sci.'
)
select distinct name
from comp_students,takes
where (grade='A' or grade='A-') and takes.id in (comp_students.id);
---b
